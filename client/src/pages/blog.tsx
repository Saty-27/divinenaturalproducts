...
function BlogListView({ 
  ...
  useEffect(() => {
    document.title = "Our Blogs | Divine Naturals";
  }, []);
        ...
        <section className="relative bg-gradient-to-r from-green-700 to-emerald-800 text-white py-16 md:py-24 text-center">
          <div className="max-w-4xl mx-auto px-4">
            ...
            <p className="text-lg md:text-xl text-green-100 font-medium max-w-2xl mx-auto">
              Read the latest stories, health tips, and updates from Divine Naturals.
            </p>
          </div>
...
function BlogDetailView({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    if (detailData?.blog) {
      document.title = `${detailData.blog.metaTitle || detailData.blog.title} | Divine Naturals`;
    }
  }, [detailData]);
