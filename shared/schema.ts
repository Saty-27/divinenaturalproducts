...
// ==========================================
// NEW CUSTOM SUBSCRIPTION PLANS & USER SUBSCRIPTIONS
// ==========================================

export const customSubscriptionPlans = pgTable("custom_subscription_plans", {
  id: serial("id").primaryKey(),
  title: varchar("title").notNull(),
  productId: integer("product_id").references(() => products.id),
  ...
  unitType: varchar("unit_type").notNull(), // Liter, Kg, Pack, Bottle, Custom
  price: decimal("price", { precision: 10, scale: 2 }).notNull(),
  originalPrice: decimal("original_price", { precision: 10, scale: 2 }),
  frequency: varchar("frequency").notNull(), // Daily, Alternate Days, Weekly, Monthly, Custom
  shortDescription: text("short_description"),
  detailedDescription: text("detailed_description"),
  benefits: jsonb("benefits").default([]), // array of bullet points
  deliveryTimeSlot: varchar("delivery_time_slot"),
  durationOptions: jsonb("duration_options").default([]), // array of strings (e.g. ['7 days', '15 days', '30 days'])
...
export const userSubscriptions = pgTable("user_subscriptions", {
  ...
  subscriptionStatus: varchar("subscription_status").notNull().default("active"), // active, paused, cancelled, completed
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

// Relations
export const customSubscriptionPlansRelations = relations(customSubscriptionPlans, ({ one }) => ({
  product: one(products, {
...
// Zod schemas
export const insertCustomSubscriptionPlanSchema = createInsertSchema(customSubscriptionPlans);
export const insertUserSubscriptionSchema = createInsertSchema(userSubscriptions);

// TypeScript types
export type CustomSubscriptionPlan = typeof customSubscriptionPlans.$inferSelect;
export type InsertCustomSubscriptionPlan = typeof customSubscriptionPlans.$inferInsert;
